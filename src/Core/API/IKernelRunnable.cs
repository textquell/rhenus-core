namespace Rhenus.Core.API
{
	...
	interface IKernelRunnable
	{
		...
		/// This method provides the type of the base task that is being wrapped by any number of KernelRunnables, where a given task that wraps 
		/// another task will return that other task's base type such that any wrapping task can be queried and will return the same base task type.
		/// </summary>
		/// <returns>
